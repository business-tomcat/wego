...
const (
	...
	openweatherCurrentURI = "http://api.openweathermap.org/data/2.5/weather?%s&appid=%s&units=metric&lang=%s"
)
...
func (c *openWeatherConfig) fetchCurrent(url string) (*openWeatherCurrentResponse, error) {
	res, err := http.Get(url)
	if c.debug {
		fmt.Printf("Fetching %s\n", url)
	}
	...
	var resp openWeatherCurrentResponse
	if err = json.Unmarshal(body, &resp); err != nil {
		return nil, fmt.Errorf("Unable to unmarshal response (%s): %v\nThe json body is: %s", url, err, string(body))
	}
	if resp.Cod != 200 {
...
func (c *openWeatherConfig) parseCondCurrent(dataInfo openWeatherCurrentResponse) (iface.Cond, error) {
	var ret iface.Cond
	codemap := map[int]iface.WeatherCode{
		200: iface.CodeThunderyShowers,
		201: iface.CodeThunderyShowers,
...
func (c *openWeatherConfig) Fetch(location string, numdays int) iface.Data {
	...
	respCurrent, errCurrent := c.fetchCurrent(fmt.Sprintf(openweatherCurrentURI, loc, c.apiKey, c.lang))
	if errCurrent != nil {
		log.Fatalf("Failed to fetch current weather data: %v\n", errCurrent)
	}
	ret.Current, errCurrent = c.parseCondCurrent(*respCurrent)

	resp, err := c.fetch(fmt.Sprintf(openweatherURI, loc, c.apiKey, c.lang))
	if err != nil {
		log.Fatalf("Failed to fetch forecast weather data: %v\n", err)
	}
